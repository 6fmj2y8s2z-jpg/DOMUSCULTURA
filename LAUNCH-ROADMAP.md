# Domus Cultura — 24-month revenue roadmap

Operating targets, not guarantees. Prioritise cash flow, proof of demand and a recognisable cultural point of view.

## Recommended stack

- Keep the editorial site on Vercel and use Shopify in CHF as the commerce, checkout and payment back end.
- Gelato is the first-choice merchandise fulfiller because its network includes Switzerland and EU production; Printful is the backup for products or branding options that test better there.
- Hahnemühle My Art Registry 2.0 is the first-choice route for certified, limited Ben Taleb editions: on-demand certified-studio production, edition registry, certificate and matching hologram. Link the Domus product page to its verified purchase route unless Hahnemühle confirms an embedded option.
- Prodigi is the direct-store fallback for Hahnemühle Photo Rag 308 gsm photography because it supports EU fulfilment and Shopify/API integration; do not use it for limited-edition direct shipping until the matching certificate/hologram workflow is contractually confirmed.
- Link each Ben Taleb product page to its verified My Art Registry record; keep one edition counter shared across all three sizes.
- Compare physical samples from Gelato, Printful and Prodigi before any public checkout opens.
- Three unframed print sizes on museum-grade Hahnemühle paper; 100 copies maximum per image across all sizes; numbered certificate.
- Launch price bands: small CHF 150–200, medium CHF 300–450, large CHF 600–800.
- SoundCloud embeds for a weekly 45–60 minute Juice+ programme.
- Instagram for discovery, email for ownership, events for trust, store for conversion.

## Zero-inventory operating model

- No speculative stock: every sellable item is produced only after a paid order.
- Fine-art photography: My Art Registry routes certified Hahnemühle production through its studio network.
- Apparel and objects: an approved Shopify print-on-demand partner fulfils T-shirts, heavyweight sweatshirts, caps, mugs and totes.
- Paper goods: postcards and small print sets are printed on demand or sold through short, fully pre-sold releases.
- Shopify remains the central storefront; each order is automatically routed to the appropriate production partner.
- Samples are the only physical items Domus Cultura buys before launch, because colour, textile weight, fit and packaging must be approved.
- Products are added slowly: one tested supplier and one approved sample per product category before public sale.

## Days 1–30

| Day | Deliverable |
|---:|---|
| 1 | Finalise the one-sentence Domus Cultura positioning. |
| 2 | Select 12 Ben Taleb photographs and mark three launch candidates. |
| 3 | Export colour-managed masters and create a metadata sheet. |
| 4 | Confirm the exact dimensions for the three unframed sizes and document the edition-of-100-across-all-sizes rule. |
| 5 | Create Shopify; reserve naming; set CHF as currency. |
| 6 | Create the Hahnemühle ID and My Art Registry artist account; order the official Certificate of Authenticity Kit and open Gelato and Prodigi test accounts. |
| 7 | Write Edition 001 story and product copy. |
| 8 | Mock up Edition 001 in a real interior. |
| 9 | Draft shipping, returns, privacy, terms and edition policies for Swiss review. |
| 10 | Set prices from landed cost, fees, taxes and target margin. |
| 11 | Register Edition 001 in My Art Registry and mirror its three formats on the Domus Cultura storefront. |
| 12 | Complete and sign the official Hahnemühle certificate; pair its numbered hologram with the artwork and prepare the Domus Cultura packaging insert. |
| 13 | Select one shirt, one tote and one cap for testing. |
| 14 | Order every launch product as a physical sample; compare one key garment with Printful if the Gelato sample is not excellent. |
| 15 | Publish the founder story: why Domus Cultura exists. |
| 16 | Produce the first gallery-visit article from the archive. |
| 17 | Record Juice+ Episode 001 and make its cover. |
| 18 | Build a permission-based list of 30 potential collectors. |
| 19 | Ask five trusted people for direct feedback, not purchases. |
| 20 | Photograph samples and packaging. |
| 21 | Approve or correct samples, colours, sizing and files. |
| 22 | Connect checkout and run a CHF 1 test transaction/refund. |
| 23 | Connect fulfilment and run an end-to-end test order. |
| 24 | Add the real SoundCloud player and Episode 001. |
| 25 | Publish three journal pieces and optimise images. |
| 26 | Prepare the seven-day launch content sequence. |
| 27 | Give collectors 24-hour private access. |
| 28 | Launch Edition 001 publicly with a live edition counter. |
| 29 | Follow up personally and log objections/questions. |
| 30 | Review sales, sign-ups, traffic and margin; decide what to repeat. |

## Months 2–3 — prove demand

- One Juice+ mix and one journal article per week.
- One photography edition per month; no oversized catalogue.
- One gallery/studio visit or listening event per month.
- Goal: 100 qualified subscribers, 10 customers, CHF 3k–8k cumulative revenue.

## Months 4–6 — make it repeatable

- Drop 02 based on the best size and price from Drop 01.
- Add two merch products only after sample approval.
- Pitch five local venues weekly for a pop-up or Juice+ night.
- Test one 30–50 unit artist collaboration with a written revenue split.
- Add B2B art placement, photography and cultural-content offers.
- Goal: CHF 12k–25k cumulative revenue.

## Months 7–12 — establish the house

- Quarterly drops, each with an event and editorial story.
- Ben Taleb commission package at CHF 1,500–4,000.
- Creative/cultural direction retainers at CHF 900–2,500/month.
- Collector CRM with interests, purchases and follow-up dates.
- Year-one objective: CHF 45k–75k.

## Months 13–18 — scale the winners

- Retire weak products; expand the strongest 20%.
- Two artist collaborations with clear rights and edition agreements.
- Corporate art placement for offices, hotels and restaurants.
- One recurring venue partnership for Domus Cultura Sound Nights.
- Six-month objective: CHF 45k–70k.

## Months 19–24 — grow beyond the founder

- Pre-order funded Domus Cultura zine.
- Two-day pop-up combining editions, talks, music and commissions.
- Wholesale terms for museum shops and concept stores.
- Collector tier with previews, visits and first refusal.
- Year-two objective: CHF 100k–160k, with a path toward CHF 200k combined annual income when services and commissions are included.

## Weekly rhythm

- Monday: pipeline, fulfilment, finances and metrics.
- Tuesday: photography or product creation.
- Wednesday: article, edit or artist conversation.
- Thursday: outreach, partnerships and proposals.
- Friday: Juice+ mix and publishing.
- Saturday: gallery, atelier and event fieldwork.
- Sunday: archive, select and plan.
